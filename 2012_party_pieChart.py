...
#load df
df = pd.read_csv('data/2012_party_clean.csv')
...
plt.title('2012年全國不分區立委選舉結果')

plt.xlabel('得票比例（獲得席次門檻：5%）')
plt.text(1.2, 0.85, '席次：\n民主進步黨：13\n中國國民黨：16\n親民黨：2\n台灣團結聯盟：3',backgroundcolor = 'white', fontsize=12, color = 'black')
fig.patch.set_facecolor('#E5C2A9')

plt.savefig('image/2012_party_pieChart.png', dpi=300, bbox_inches='tight')
plt.show()